...
pub fn bytes_to_ethereum_address(addr_bytes: &[u8]) -> Result<String, StdError> {
	let Some(addr_bytes_fixed): Option<&[u8; 20]> = addr_bytes.last_chunk() else {
		return Err(StdError::serialize_err("EthereumAddress", "address was less than 20 bytes in length"));
	};
...
pub fn checksumify_ethereum_address(addr_str: &mut str) -> Result<(), StdError> {
	...
	if addr_prefix != "0x" {
		return Err(StdError::generic_err("checksumify_ethereum_address: address does not start with 0x"));
	}
	if addr_hex.len() != 40 {
		return Err(StdError::generic_err("checksumify_ethereum_address: address is not 42 bytes long"));
	}
	// Technically too permissive, but don't have the time to write something as efficient as this impl
	if !addr_hex.is_ascii() {
		return Err(StdError::generic_err("checksumify_ethereum_address: address contains invalid characters"));
	}
