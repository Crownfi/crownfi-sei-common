use std::{collections::{BTreeSet, BTreeMap}, sync::Arc, borrow::Cow};

use convert_case::{Casing, Case};
use deunicode::deunicode_with_tofu_cow;
use lazy_regex::{{regex, regex::Replacer}, Regex, Captures};
use schemars::schema::{Schema, InstanceType, ObjectValidation};

use crate::{error::SdkMakerError, struct_extentions::{SchemaStructExtentions, SingleOrVecStructExtentions}};

/// makes the first letter uppercase
pub(crate) fn upper_first_ascii<'a>(
	input: &mut Cow<'a, str>
) {
...
fn mut_cow_str_replace_all<'a>(
	cow_str: &'_ mut Cow<'a, str>,
	regex: &Regex,
...
pub(crate) fn make_type_name<'a>(
	...
	mut_cow_str_replace_all(&mut txt, regex!(r"^_[a-z]"), |m: &Captures<'_>| {m[0].to_ascii_uppercase()});
	// remove non-leading underscores followed by lowercase (convert snake_case)
	mut_cow_str_replace_all(&mut txt, regex!(r"_[a-z]"), |m: &Captures<'_>| {m[0][1..].to_ascii_uppercase()});
	// uppercase letters after digits, dollars
	mut_cow_str_replace_all(&mut txt, regex!(r"(?:[\d$]+[a-zA-Z])"), |m: &Captures<'_>| {m[0].to_ascii_uppercase()});
	// uppercase first letter after whitespace
	mut_cow_str_replace_all(&mut txt, regex!(r"\s+([a-zA-Z])"), |m: &Captures<'_>| {m[0].trim().to_ascii_uppercase()});
...
pub(crate) fn schema_type_string(
	...
	if let Some(schema_object) = schema.as_object() {
		if let Some(schema_object_array) = schema_object.array.as_ref() {
			let Some(sub_type) = schema_object_array.items
				.as_ref()
				.and_then(|array_items| {array_items.as_single()})
			else {
				return Err(
					SdkMakerError::EnumVariantFieldHasMultiTypedArray(
			...
			if let Some(array_length) = schema_object_array.max_items.filter(|max_items| {
				*max_items == schema_object_array.min_items.unwrap_or_default()
			}) {
		...
		// Nullable type references
			...
			.and_then(|multi_type| {
				...
				if !nullable_type.as_object()
					.and_then(|v| {{
						v.instance_type.as_ref()
					}})
					.and_then(|instance_type| {
						instance_type.as_single()
					})
...
impl MethodGenType<'_> {
	...
	pub(crate) fn generate_method_name(
		...
		match self {
			MethodGenType::Instantiate => {
				"instantiateIx".to_string()
			},
			...
			MethodGenType::Query(_) => {
				[
					"query",
					&enum_variant.to_case(Case::Pascal)
				].join("")
			},
			MethodGenType::Migrate => {
				"migrateIx".to_string()
			},
			MethodGenType::Sudo => {
				[
	...
	pub(crate) fn parent_func_call(&self) -> &'static str {
		match self {
			MethodGenType::Instantiate |
			MethodGenType::Migrate |
			MethodGenType::Sudo => todo!("Unknown parent function for {:?}", self),
			MethodGenType::Execute => {
				"this.executeIx(msg, funds)"
			},
			MethodGenType::Query(_) => {
				"this.query(msg)"
			},
			MethodGenType::Cw20Hook => {
				"this.executeIxCw20(msg, tokenContractOrUnifiedDenom, amount)"
			},
		}
	}
